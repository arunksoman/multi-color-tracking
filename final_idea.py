...
def reverse(right_pwm, left_pwm):
    rightMotorFwd.ChangeDutyCycle(0)
    ...
    for (lower, upper, colorName) in colorRanges:
        ...
        if firstScene == 0 and OrangeIndicate and greenIndicate:
            ...
            if (xB_axis < leftBound) or (xB_axis > rightBound):
                ...
                pwmOut = abs(error * kp)
                turnPwm = (pwmOut + defaultSpeed  * 1.2)
            ...
            if (x_axis < leftBound) or (x_axis > rightBound):
                ...
                if x_axis < (leftBound):
                    ...
                    if xB_axis > 620:
                        ...
                        UpdatePwm(defaultSpeed, turnPwm)
            elif radius > Green_thresh:
                pwmStop()
                if xB_axis > x_axis:
                    spinRight()
                    if not GreenIndicate and OrangeIndicate:
                        pwmStop()
                        robot_controller() ## Propotional controller
                if xB_axis < x_axis:
                    spinLeft()
                    if not GreenIndicate and OrangeIndicate:
                        pwmStop()
                        robot_controller()
        if firstScene == 1 and OrangeIndicate and GreenIndicate:
            robot_controller()
        if firstScene == 1 and radius > orangeThreshold:
            if x_axis > xB_axis:
                robotRight() # code for turning towards right
            if x_axis < xB_axis:
                robotLeft() # code for turning towards left
